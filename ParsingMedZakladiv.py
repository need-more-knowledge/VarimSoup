...
#
#             if not driver.find_elements(By.CLASS_NAME,"hasmore-text"):
...
#Збираємо всі посилання на клініки і зберігаємо в текстовик
...
#
#     with open("UrlList.txt", "w") as file:
#         for url in urls:
#             file.write(f"{url}\n")
#
...
# Пишимо функцію яка вичитає тектовик и збере список вичещених від пробелів посилань
file_path2 = "UrlList.txt";
def get_data(file_path2):
    with open(file_path2) as file:
        ...
        # Тут написано то саме, що і в коді з 73 по 79 лінії, але коротше і краще
        urls_list = [url.strip() for url in file.readlines()]

# Додаєм хедери і юзерагент і в циклі пробігаємо по кожному посиланні і збираємо данні
    result_list = [] #все що знайшли - наповнюєм сюди
    ...
    for url in urls_list:
        ...
        try:
            item_name = soup.find("span", {"itemprop": "name"}).string.strip()
        except Exception as _ex:
            item_name = None
        ...
        try:
            ...
            # забираємо телефон повністю "tel:+79813361632" і розділяємо по : забираючи праву частину із номером
        ...
        try:
            item_social_networks = soup.find("div",class_="js-service-socials").find('div',class_="z-text--13").find_all("a");

            for sn in item_social_networks:
                sn_url = sn.get("href")
                sn_url = unquote(sn_url.split("?to=")[1].split("&")[0])
                #Наповнюєм список чистим урлом
